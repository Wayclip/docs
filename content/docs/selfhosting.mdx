...
services:
  api_wayclip:
    image: wayclip/api:latest
    container_name: api_wayclip
    pull_policy: always
    env_file:
      - .env
    ports:
    ...
    entrypoint: >
      sh -c '
        ...
        sleep 5
        echo "Running database migrations from /usr/src/app/migrations..."
        sqlx migrate run --source /usr/src/app/migrations --database-url "$$DATABASE_URL"
        echo "Migrations complete. Starting API..."
        /usr/local/bin/wayclip_api
      '

  db_wayclip:
    image: postgres:16-alpine
    container_name: db_wayclip
    environment:
    ...
    entrypoint: |
      sh -c '
        while true; do
          ...
          pg_dump -Fc > /backups/wayclip_db_backup_$(date +%Y-%m-%d_%H-%M-%S).dump
...
POSTGRES_PORT=5432
POSTGRES_DB=wayclip
DATABASE_URL=postgres://admin:XxXxXxXxXxXxXxXxXxXxXxXx@wayclip_db:5432/wayclip
REDIS_URL=redis://localhost:6379

# -----------------------------
# General Server Settings
# -----------------------------
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/0000000000000000000/XxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXxXx
...
# -- LOCAL (only if STORAGE_TYPE=LOCAL)
...
SFTP_PUBLIC_URL=https://example.com
...
<Callout title='Server Port'>
    The API runs on port `8080` inside the container, which is mapped to port `8008` on your host machine. If you use a
    reverse proxy like Nginx, you should proxy requests to `http://127.0.0.1:8008`.
</Callout>
...
#### Very sorry, this is AI generated as a filler text (I promise I will update it) (But the info is true)
